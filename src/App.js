      ...
      <div style={{ maxWidth: '400px', margin: 'auto', marginTop: '40px', padding: '20px', backgroundColor: '#fff', borderRadius: '8px', boxShadow: '0 2px 8px rgba(0,0,0,0.1)' }}>
        {!isLoggedIn ? (
          ...
          <div>
            ...
            <h3>Richieste degli utenti</h3>
            {requests.length === 0 ? <p>Nessuna richiesta</p> : (
              <ul>
                {requests.map((req) => (
                  <li key={req.id}>
                    <strong>{req.description}</strong> - <em>{req.status}</em>
                    <button onClick={() => setRequests(requests.map(r => r.id === req.id ? { ...r, status: 'Completato' } : r))}>Completa</button>
                  </li>
                ))}
              </ul>
            )}
